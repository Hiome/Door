...
for (idx_t fidx=0; fidx < MAX_PEOPLE; fidx++) {
  ...
  float score = sq(d/maxDpoint) + sq(tempDiff/maxTpoint);
  score -= (0.01*((float)(forgotten_people[fidx].neighbors)));
  score -= (float(forgotten_people[fidx].confidence)/float(points[i].confidence*10.0));
